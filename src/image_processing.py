...
def preprocessing(img):
    img_gray = cvtColor(img, COLOR_BGR2GRAY)                 # Convert the input image to a grayscale
    img_threshold = adaptiveThreshold(src=img_gray, maxValue=255, \
...
def draw_lines(img, lines):
    ...
    for dline in lines:
        ...
        x2 = int(x0 - 10000*(-b))
        y2 = int(y0 - 10000*(a))
        line(img_lines,(x1,y1),(x2,y2),(0,0,255),2)
...
def draw_points(img, points):
    ...
    for point in points:
        circle(img_points,point, 10,(0,255,0),-1)
...
def extract_lines(img):
    ...
    extracted_lines = HoughLines(img, 1, np.pi/180, min_votes)
